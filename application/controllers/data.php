...
class Data_Controller extends Base_Controller
{
    ...
    public function post_firms()
    {
        ...
        $donation = new Firm();
        $donation->fill(Input::get()); // testen ob das funzt
        $donation->save();        
        
        if( isset($name1) ) {
            Event::fire('firm_add', array(
                $name1,
                Auth::user()
            ));
            Session::flash('success', 'Firma erfolgreich angelegt');
        } else {
            Session::flash('error', 'Bitte geben Sie korrekte Werte an');
        }
